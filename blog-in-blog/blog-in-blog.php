...
include_once( WP_PLUGIN_DIR . "/blog-in-blog/blocks/blog-in-blog-block.php" );
 ...
 */
function blog_in_blog_activate() {
    // Initialize default options
    bib_init_opts();
    // Store the plugin version
    update_option('bib_version', BIB_VERSION);
}
register_activation_hook(__FILE__, 'blog_in_blog_activate');

/**
 * Plugin deactivation hook.
 * Performs cleanup when the plugin is deactivated.
 * Note: Options are preserved - they are only deleted on uninstall.
 */
function blog_in_blog_deactivate() {
    // Nothing to do on deactivation - options preserved for reactivation
    // Uninstall.php handles full cleanup when plugin is deleted
}
register_deactivation_hook(__FILE__, 'blog_in_blog_deactivate');
...
function blog_in_blog_func($atts) {
    ...
    // Allow rendering on pages, in REST API (for Gutenberg), and in admin (for previews)
    $is_rest_request = defined('REST_REQUEST') && REST_REQUEST;
    $is_admin_request = is_admin();
    if (!is_page() && !$is_rest_request && !$is_admin_request) {
    ...
    // set some values from the shortcode (with sanitization)
    $blog_in_blog_opts['cat'] = sanitize_text_field($atts['category_id']);
    $blog_in_blog_opts['cat_slug'] = sanitize_title($atts['category_slug']);
    $blog_in_blog_opts['tag_slug'] = sanitize_title($atts['tag_slug']);
    $blog_in_blog_opts['custom_post_type'] = sanitize_key($atts['custom_post_type']);
    $blog_in_blog_opts['num'] = absint($atts['num']);
    $blog_in_blog_opts['post_order'] = bib_set_post_order(sanitize_key($atts['sort']));
    $blog_in_blog_opts['order_by'] = sanitize_key($atts['order_by']);
    $blog_in_blog_opts['custom_order_by'] = sanitize_text_field($atts['custom_order_by']);
    $blog_in_blog_opts['post_id'] = $atts['post_id'] !== '' ? absint($atts['post_id']) : '';
    $blog_in_blog_opts['pagination'] = sanitize_key($atts['pagination']);
    $blog_in_blog_opts['template'] = sanitize_file_name($atts['template']);
    $blog_in_blog_opts['author'] = $atts['author'] !== '' ? absint($atts['author']) : '';
    $blog_in_blog_opts['author_name'] = sanitize_user($atts['author_name']);
    $blog_in_blog_opts['hidefirst'] = absint($atts['hidefirst']);
    ...
    if (strstr($thumbnail_size, 'x')) {
        $blog_in_blog_opts['thumbnail_size'] = explode('x', $thumbnail_size);
    } else {
        $blog_in_blog_opts['thumbnail_size'] = $thumbnail_size;
    }

    // Initialize template comment (will be prepended to output, but not during REST requests)
    $template_comment = '';
    ...
    if ($template != '') {
        ...
        if (file_exists(BIB_WP_UPLOADS_DIR . "/" . $template)) {
            ...
            $template_comment = "<!-- BIB: using template: " . esc_html($blog_in_blog_opts['bib_post_template']) . " -->" ;
        ...
        $blog_in_blog_opts['bib_post_template'] = ''; // this will force using bib_html from database.
        $template_comment = "<!-- BIB: using default template from database -->" ;
    ...
    if (is_array($postslist)){
        ...
        foreach ($postslist as $post) {
            ...
            $data['post_author'] = $user ? apply_filters('the_author', $user->display_name) : '';
            ...
            if (is_array($cats)){
                foreach ($cats as $v) {
                    ...
                    $catstr .= ' <a href="' . esc_url($cat_link) . '" title="' . esc_attr($v->cat_name) . '" >' . esc_html($v->cat_name) . '</a>' . esc_html($blog_in_blog_opts['bib_text_delim']);
                }
            }
    ...
    // Include template comment only for non-REST requests (front-end display)
    if (!$is_rest_request && !empty($template_comment)) {
        $out = $template_comment . $out;
    }
...
function bib_get_posts() {
    ...
    if ($blog_in_blog_opts['post_id'] == '') { // for multiposts
        ...
        $postslist[0] = get_post($blog_in_blog_opts['post_id']);
...
function bib_filter_excerpt_more($more, $excerpt=''){
    ...
    // Only apply our custom excerpt_more when Blog in Blog is rendering
    if (!isset($blog_in_blog_opts['current_post_id']) || empty($blog_in_blog_opts['current_post_id'])) {
        return $more;
    }
...
// don't know which post is calling the shortcode.
    ...
    $host_page = isset($blog_in_blog_opts['host_page']) ? $blog_in_blog_opts['host_page'] : 'unknown';
    bib_write_debug( __FUNCTION__,"Host Page ID: ".$host_page);

    //if ($wp_query->is_home()){
    //    bib_write_debug(__FUNCTION__,"HOME PAGE!!!");
    // get the REQUEST_URI
    $perma_link = isset($_SERVER['REQUEST_URI']) ? esc_url_raw($_SERVER['REQUEST_URI']) : '';
...
function bib_get_post_count() {
    ...
    // Check if we are getting custom post types only (without category filters)
    if ($blog_in_blog_opts['custom_post_type'] != ''
            && $blog_in_blog_opts['cat'] == ''
            && $blog_in_blog_opts['cat_slug'] == '' ){

        $post_type = sanitize_key($blog_in_blog_opts['custom_post_type']);
        ...
        // Base query for category/tag based counts
        $querystr = "SELECT COUNT(DISTINCT {$wpdb->posts}.ID)
            FROM {$wpdb->term_taxonomy}, {$wpdb->posts}, {$wpdb->term_relationships}, {$wpdb->terms}
            WHERE {$wpdb->posts}.ID = {$wpdb->term_relationships}.object_id
            AND {$wpdb->term_relationships}.term_taxonomy_id = {$wpdb->term_taxonomy}.term_taxonomy_id";

        // If there are categories by ID
        if ($blog_in_blog_opts['cat'] != '') {
            if (stristr($blog_in_blog_opts['cat'], ',')) {
                // Multiple category IDs - sanitize each one
                $cat_ids = array_map('absint', explode(',', $blog_in_blog_opts['cat']));
                $cat_ids = array_filter($cat_ids); // Remove zeros
                if (!empty($cat_ids)) {
                    $placeholders = implode(',', array_fill(0, count($cat_ids), '%d'));
                    $querystr .= " AND {$wpdb->term_taxonomy}.term_id IN ($placeholders)";
                    $prepare_values = array_merge($prepare_values, $cat_ids);
                }
            } else {
                // Single category ID
                $querystr .= " AND {$wpdb->term_taxonomy}.term_id = %d";
                $prepare_values[] = absint($blog_in_blog_opts['cat']);
            }
        }

        // If there is a category slug
        if ($blog_in_blog_opts['cat_slug'] != '') {
            $querystr .= " AND {$wpdb->terms}.term_id = {$wpdb->term_taxonomy}.term_id
                AND {$wpdb->terms}.slug = %s";
            $prepare_values[] = sanitize_title($blog_in_blog_opts['cat_slug']);
        }

        // If there is a custom post_type involved
        if ($blog_in_blog_opts['custom_post_type'] != ''){
            $querystr .= " AND {$wpdb->posts}.post_type = %s";
            $prepare_values[] = sanitize_key($blog_in_blog_opts['custom_post_type']);
        }

        // If there is an author involved
        if ($blog_in_blog_opts['author'] != '' || $blog_in_blog_opts['author_name'] != '' ){
            if($blog_in_blog_opts['author'] != ''){
                $querystr .= " AND {$wpdb->posts}.post_author = %d";
                $prepare_values[] = absint($blog_in_blog_opts['author']);
            }
            if($blog_in_blog_opts['author_name'] != ''){
                $author = get_user_by('slug', sanitize_user($blog_in_blog_opts['author_name']));
                if ($author) {
                    bib_write_debug(__FUNCTION__, print_r($author,true));
                    $querystr .= " AND {$wpdb->posts}.post_author = %d";
                    $prepare_values[] = $author->ID;
                }
            }
        }
        ...
        // Prepare the query if we have values to prepare
        if (!empty($prepare_values)) {
            $querystr = $wpdb->prepare($querystr, $prepare_values);
        }
    }
...
function bib_show_debug() {
    ...
    $OPT = get_option('bib_debug');
    if ($OPT){
        $debug_output = isset($blog_in_blog_opts['debug_output']) ? $blog_in_blog_opts['debug_output'] : '';
        $output = "<br /><h2>BLOG_IN_BLOG DEBUG INFO</h2><small>Turn this off in the 'Misc' section of the blog_in_blog admin page.</small><br /><textarea cols='100' rows='20'>" . esc_textarea($debug_output) . "</textarea>";
        unset ($blog_in_blog_opts['debug_output']);
        echo $output ;
    }
}

add_action( 'init', 'blog_in_blog_load_textdomain' );
/**
 * Load plugin textdomain.
 */
function blog_in_blog_load_textdomain() {
    load_plugin_textdomain( 'blog-in-blog', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}
?>
